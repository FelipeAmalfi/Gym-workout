...
export const getSystemPrompt = () => JSON.stringify({
    ...
    scenarios: {
        ...
        get_workout_success: 'Present the full workout details: name, difficulty, goal, then each exercise with sets/reps in order. If details.actionData has a "matches" array, the user asked by muscle group and several workouts matched — render them as a numbered list (position. Name — Muscles — Difficulty), no IDs, and invite them to ask for one specifically.',
        get_workout_error: 'Explain that the workout was not found. Suggest listing workouts to see what is available.',
        list_workouts_success: 'List all workouts as a numbered list: position. Name — Difficulty — Goal. Do NOT show database IDs. If empty, encourage creating the first one.',
